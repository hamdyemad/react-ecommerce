...
export function CategoriesPage() {
  ...
  const fetchCategories = async (pageNum: number, append: boolean = false, currentSearch: string = searchTerm) => {
    try {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setPage(1);
      fetchCategories(1, false, searchTerm);
    }, 500);
  ...
  return (
    <div className="min-h-screen py-8 px-4 sm:px-6 max-w-[1440px] mx-auto">
      <SEO title={t('common:categories', 'Categories')} />
      <BreadCrumb 
        items={[
          { label: t('common:home', 'Home'), path: '/' },
          { label: t('common:categories', 'Categories'), path: '/categories' }
        ]}
      />
      ...
      <div className="text-center mt-4 mb-6 relative py-4">
        ...
        >
          {t('common:browseCategoriesDesc', 'Discover our wide range of carefully curated products across all categories.')}
        </p>
      </div>
      ...
      <div className="flex justify-center mb-12 px-4 sm:px-10">
        <div className="relative w-full md:max-w-2xl group">
          <input 
            type="text"
            placeholder={t('common:searchCategories', 'Search categories...')}
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="text-center py-24 px-6 rounded-[50px] border border-dashed mb-12" style={{ borderColor: tokens.colors[mode].border.DEFAULT }}>
          ...
          <p className="max-w-md mx-auto font-bold opacity-60 text-lg" style={{ color: tokens.colors[mode].text.secondary }}>
            {t('common:searchNoResultsDesc', "We couldn't find any categories matching your search. Try different keywords.")}
          </p>
          <button 
                      ...
                      >
                        ...
                        >
                          ...
                          {cat.icon ? (
                            ...
                            <span className="text-5xl sm:text-7xl group-hover:scale-115 transition-transform duration-700">🗂️</span>
                          )}
                        </div>
                        ...
                        <div className="w-full">
                          ...
                          <div className="flex flex-col items-center">
                            <span className="inline-block px-6 sm:px-8 py-2 sm:py-3 rounded-full bg-primary/10 text-primary text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] shadow-sm transform group-hover:translate-y-[-2px] transition-transform duration-500">
                              {cat.products_count || 0} {t('common:items', 'Items')}
                            </span>
                          </div>
                        </div>
                      </div>
