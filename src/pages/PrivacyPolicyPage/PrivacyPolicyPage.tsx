  ...
  return (
    <div className="min-h-screen py-8">
      <BreadCrumb 
        items={[
          { label: t('home'), path: '/' },
          { label: t('privacySubtitle'), path: '/privacy-policy' }
        ]}
      />
      ...
      {/* Content Section */}
      <div 
        className="max-w-5xl mx-auto p-10 md:p-16 rounded-[40px] transition-all duration-500 shadow-2xl relative overflow-hidden"
        style={{
      ...
      >
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            <p className="text-2xl font-black italic opacity-30" style={{ color: tokens.colors[mode].text.tertiary }}>{t('noPrivacyContent')}</p>
          </div>
        )}
      </div>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-pink-900/30 rounded-full translate-y-1/2 -translate-x-1/2 blur-[80px] pointer-events-none" />
      </div>
    </div>
  );
}
