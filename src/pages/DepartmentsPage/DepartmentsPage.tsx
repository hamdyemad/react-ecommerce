...
export function DepartmentsPage() {
  ...
  const fetchDepartments = async (pageNum: number, append: boolean = false, currentSearch: string = searchTerm) => {
    try {
      ...
      if (append) {
        setDepartments(prev => [...prev, ...newItems]);
      } else {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setPage(1);
      fetchDepartments(1, false, searchTerm);
    }, 500);
  ...
  return (
    <div className="min-h-screen py-8 px-4 sm:px-6 max-w-[1440px] mx-auto">
      <SEO title={t('common:departments', 'Departments')} />
      <BreadCrumb 
        items={[
          { label: t('common:home', 'Home'), path: '/' },
          { label: t('common:departments', 'Departments'), path: '/departments' }
        ]}
      />
      ...
      <div className="text-center mt-4 mb-6 relative py-4">
        ...
        >
          {t('common:browseDepartmentsDesc', 'Discover categories and products organized by departments.')}
        </p>
      </div>
      ...
      <div className="flex justify-center mb-12 px-4 sm:px-10">
        <div className="relative w-full md:max-w-2xl group">
          <input 
            type="text"
            placeholder={t('common:searchDepartments', 'Search departments...')}
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="text-center py-24 px-6 rounded-[50px] border border-dashed mb-12" style={{ borderColor: tokens.colors[mode].border.DEFAULT }}>
          ...
          <p className="max-w-md mx-auto font-bold opacity-60 text-lg" style={{ color: tokens.colors[mode].text.secondary }}>
            {t('common:searchNoResultsDesc', "We couldn't find any departments matching your search. Try different keywords.")}
          </p>
          <button 
        ...
        <div className="mb-2 relative">
          ...
          >
            {(() => {
              const slides = [];
              for (let i = 0; i < departments.length; i += itemsPerSlide) {
                slides.push(departments.slice(i, i + itemsPerSlide));
              }
              return slides.map((slideDepts, idx) => (
                          ...
                          <div className="flex flex-col items-center gap-2">
                             <div className="flex gap-2">
                                <span className="inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary text-[10px] font-black uppercase tracking-wider">
                                  {dept.products_count || 0} {t('common:products', 'Products')}
                                </span>
                             </div>
                             <span className="text-xs font-bold opacity-40 uppercase tracking-widest">{dept.categories_count || 0} {t('common:categories', 'Categories')}</span>
                          </div>
                        </div>
                      </div>
                    </Link>
